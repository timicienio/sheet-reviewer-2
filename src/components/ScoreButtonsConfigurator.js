...
export default function ScoreButtonConfigurator({
  ...
  return (
    <>
      <Modal show={showEdit}>
        <Modal.Header>
          <Modal.Title>Editing sub-score #{editingIndex + 1}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <h5>
            Label{" "}
          ...
          <InputGroup className="mb-3">
            ...
            <InputGroup.Append>
              <Button
                onClick={() => onChangeTag(editingIndex, editingTagValue)}
              >
                Save
              </Button>
            </InputGroup.Append>
            ...
            <>
              <h5>
                ...
                <ReactTooltip id="range-tooltip">
                  <span>
                    Max. and min. value of number score; largest range: 10
                  </span>
                </ReactTooltip>
              </h5>
              ...
              >
                <ButtonGroup>
                  ...
                  <Button
                    ...
                    disabled={
                      state[editingIndex]?.upper - state[editingIndex]?.lower >=
                      10
                    }
                  >
                    ⬇
                ...
                <ListGroup horizontal style={{ margin: "0 10px 0 10px" }}>
                  <ListGroup.Item>{state[editingIndex]?.lower}</ListGroup.Item>
                  <ListGroup.Item> to </ListGroup.Item>
                  <ListGroup.Item>{state[editingIndex]?.upper}</ListGroup.Item>
                </ListGroup>{" "}
                <ButtonGroup>
                  <Button
                    onClick={() =>
          ...
          <h5>
            ...
            <ReactTooltip id="column-tooltip">
              <span>
                The reviewer will post the scores on the selected column. Select
                'None' if not needed.
              </span>
            </ReactTooltip>
            <Dropdown style={{ marginTop: "10px" }}>
              ...
              <Dropdown.Menu>
                <Dropdown.Item
                  onSelect={() => onChangeSheetColumn(editingIndex, null)}
                >
                  None
                </Dropdown.Item>
                {sheetColumns.map((item, index) => (
                  <Dropdown.Item
                    onSelect={() => onChangeSheetColumn(editingIndex, item)}
                  >
                    {item}
                  </Dropdown.Item>
                ))}
                  ...
                  onScore={(newScore) =>
                    setCurrentScores(
                      currentScores.map((score, scoreIndex) =>
                        index === scoreIndex ? newScore : score
                      )
                    )
                  }
              ...
              <Col>
                <ButtonGroup>
                  <DropdownButton
                    ...
                    variant={"secondary"}
                    title={"Type"}
                    onSelect={(value) => onChangeType(index, value)}
                  >
                    {["Number", "Yes-no"].map((buttonType, buttonIndex) => (
                      <Dropdown.Item
                        eventKey={buttonType}
